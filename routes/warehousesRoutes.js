...
const validateWarehouseData = (req, res, next) => {
    const {
        warehouse_name, address, city, country, contact_name,
        contact_position, contact_phone, contact_email
    } = req.body;
    if (!warehouse_name || !address || !city || !country || !contact_name
        || !contact_position || !contact_phone || !contact_email)
    {
...
router.put('/:id', validateWarehouseData, async (req, res, next) => {
    const { id } = req.params;
    const {
        warehouse_name,
        address,
